 ...
 * is horizontal and centered at the origin (0,0). An ellipse can become vertical in the case that y² is larger than x²
 * in the equation. An ellipse is can be horizontally and/or vertically shifted from the origin, when this happens
 * the ellipse comes in the form (x-h)²/a² + (y-k)²/b² = 1, with h and k representing the shifts from the x and y
 * axes respectively. The graphs of ellipses have many properties such as the vertices, the foci and the major
 * and minor axes. This program asks the user whether for the a² and b² values of the ellipse, it also asks 
 ...
 * and one if it isn't. - The program then asks if the denominator of x² is larger in the equation or not, so as to know
 * whether or not the ellipse is vertical. Then an ellipse object is instantiated with the x and y values and a boolean 
 * value that is set to true if x²'s denominator is larger in the equation. The ellipse is instantiated with a different 
